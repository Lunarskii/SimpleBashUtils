...
void appendPattern(options *flags, char *expression) {
  ...
  flags->pattern = realloc(
      flags->pattern, (patternSize + expressionSize + temp) * sizeof(char));
      ...
      case 'f':
        ...
        if ((file = fopen(optarg, "rb")) == NULL) {
          fprintf(stderr, "./s21_grep: %s: No such file or directory\n",
                  optarg);  // проверить на работу флага -s здесь на маке
        } else {
...
int print(options *flags, counters *count, char *fileName,
          unsigned long long int countStrings, char *buffer) {
...
int printFileName(options *flags, counters *count, char *fileName) {
  if ((flags->outputMatchingFiles || count->quantityFiles)) {
    if (flags->outputMatchingFiles) {
      printf("%s", fileName);
      return 0;
    } else if (!(flags->outputQuantityMatchingStrings) &&
               !(flags->outputMatchingStringsWithoutFilename)) {
...
void printStringNumber(options *flags, unsigned long long int countStrings) {
  if (flags->outputNumbersFoundRows && !(flags->outputMatchingFiles) &&
      !(flags->outputQuantityMatchingStrings)) {
...
int o_option(options *flags, counters *count, char *fileName, int countStrings,
             char *buffer, regex_t *regex, regmatch_t *pmatch) {
  count->oEnabled = true;
  if (printFileName(flags, count, fileName) == 0) {
    printf("\n");
    return 0;
...
int GrepWorkingWithArgs(options *flags, counters *count, char *fileName,
                        FILE *file, regex_t *regex) {
  ...
  while (getline(&buffer, &length, file) > 0) {
    ...
    if (flags->invert && regexec(regex, buffer, 1, &pmatch, 0)) {
      ...
      if (flags->printMatchingNoEmptyStrings &&
          !flags->outputQuantityMatchingStrings) {
        if (o_option(flags, count, fileName, countStrings, buffer, regex,
                     &pmatch) == 0) {
          if (buffer != NULL) free(buffer);
          return 0;
        }
...
int GrepWithArgs(int argc, char *argv[], options flags) {
  counters count = {0, 0, 0, 0};
  regex_t regex;
  char **end = &argv[argc];

  if (flags.patternAvailable == 0) {  // без -e, когда один шаблон
  ...
  } else if (flags.patternAvailable > 0) {  // с -e, когда много шаблонов
  ...
  for (char **fileName = argv + (flags.patternAvailable ? 0 : 1);
